...
# 내장 함수 len() : 순차형 자료(sequence)의 내부 데이터 개수를 구한다
...
- find()는 앞에서부터, rfind()는 뒤에서부터 탐색한다.
'''
...
# program이 문자열 변수 s에 있다면 해당 단어의 첫번째 인덱스 반환
print(s.find('program'))

# 탐색 시 문자열을 발견하지 못했다면 -1 반환 
print(s.find('메롱'))


# 메서드 count() : 문자열 내부에 찾는 단어의 출연 횟수 반환
message = """
    내가 그린 기린 그림은 목이 긴 기린 그림이고
    내가 그린 기린 그림은 목이 안 긴 기린 그림이다
"""

print('"기린" 단어의 출연 횟수 : ', message.count('기린'))
