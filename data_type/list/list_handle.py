...
- 리스트는 시퀀스 자료형이기 때문에 인덱스를 통한 요소들의 관리가 가능하다.
...
'''
- 문자열은 상수 형태로 저장되는 고정형 리스트이다.

- 따라서 인덱싱이나, 슬라이싱을 통해 값의 복사본을 활용하는 것은 가능하지만,
    영역에 직접 접근해서 내부의 값을 편집할 수는 없다.

- 문자열은 변경이 불가능한 자료형이다. (immutable)
'''
s = 'python'
# s[2] = 'x'
...
# 자동으로 변수에 리스트 내부 요소의 값들이 차례대로 할당된다
# javascript의 디스트럭처링(구조 분해 할당)과 유사
