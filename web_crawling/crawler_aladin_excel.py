...
# 브라우저 내부 대기
# t.sleep() -> 브라우저 로딩에 상관없이 무조건 10초 대기

# 웹 페이지 전체가 로딩될 때까지 대기 후 렌더링이 완료디었다면 남은 시간 무시
...
# 셀 너비 조정
# worksheet.set_column(시작열, 끝 열 번호, 길이, 서식 양식(format))
# 셀 높이 조정
# worksheet.set_row(행번호, 높이, 서식 양식(format))
...
'''
# enumerate 함수는 순서가 있는 자료형(리스트, 튜플, 문자열)을 전달 받아
# 인덱스와 해당 항목을 포함하는 튜플을 반환하는 함수이다
...
while index <= 9:
    ...
    for div_ss_book_box in div_list:
        ...
        # 작가
        # 위에서 얻은 title의 부모요시 li의 다음 형제 li를 지목 -> 작가, 출판사, 출판일 존재
        ...
        # 리스트 내부의 요소들에게 일괄적으로 적용할 수식, 조건, 함수의 리턴값을 설정할 수 있게 하는 문법
        '''
        li = [1, 2, 3, 4, 5]
        ...
        # 책 상세 정보 페이지 링크
        # title이라는 변수에 a 태그를 취득해놓은 상태
        # title -> a 태그의 요소 전부를 갖고 있는 상태
        # href로 작성된 키를 전달하고, 해당 value를 받아 변수에 저장
        page_link = title['href']

        try :
            # 이미지 바이트 변환 처리
            # BytesIO 객체 생성. 생성자의 매개값으로 준비해놓은 img 태그의 src 값을 urlopen 메서드에 전달
            ...
            # 파이썬에서는 블록 구조에 아무것도 작성하지 않으면 에러
            # 블록 구조 내부에 작성할 코드가 없다면 pass 키워드 사용
            pass
    ...
    # 다음 페이지(탭)로 전환
    index += 1
    driver.find_element(By.XPATH, f'//*[@id="newbg_body"]/div[3]/ul/li[{index}]/a').send_keys(Keys.ENTER)
    del soup
