...
'''
- with문을 사용하면 with 블록을 벗어나는 순간 객체가 자동으로 해제된다
    (자바의 try-with-resources와 유사하다)
    finally에 clise를 할 필요가 없다
...
- 웹이나 다른 프로그램의 텍스트 데이터와
    ...
    내장 함수 open()이 제대로 인코딩을 적용할 수 없어서
    에러가 발생한다. (UnicodeEncodeError)

- 파일 입/출력 시 인코딩 코덱을 변경하고 싶다면 codecs 모듈을 사용한다.
'''
