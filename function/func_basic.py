...
- 함수는 지속적으로 사용되는 코드 블록에 이름을 붙여놓은 형태이다.

- 함수는 한 번 정의해 두면 지정된 함수 이름을 통해
    언제든지 해당 코드 블록을 실행할 수 있다.

- 함수를 정의할 때 사용하는 키워드는 def 이다.
...
- 인수는 함수를 호출할 때 함수 실행에 필요한 값, 함수의 호출 시 전달해주는 값
    그 값을 받아내는 변수를 매개변수 (parameter)라고 한다.
...
- 인수는 여러 개 존재할 수 있지만, 반환값은 하나만 존재해야 한다.

- 모든 함수가 반환값이 있는 것은 아니다.
    함수 실행 후 반환할 값이 없다면 return을 생략할 수 있다.
'''
...
# 리턴이 있는 함수는 다른 함수의 매개값으로도 사용 가능
print(add(add(5, 7), add(9, 8))) # add(12, 17)
# n = int(input('정수: '))


def operate_all(n1, n2):
    return n1 + n2, n1 - n2, n1 * n2, n1 / n2   # tuple은 소괄호 생략 가능
    # return n1 - n2
    # return n1 * n2
    # return n1 / n2
